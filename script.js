...
let elemDiv = document.querySelector(".bg");

let theme = window.localStorage.getItem("theme");

if (theme) {
    elemDiv.style.backgroundColor = theme;
} else {
    elemDiv.style.backgroundColor = "white";
}

function funcBtnGreen() {
    window.localStorage.setItem("theme", "#c8b6ff");
    window.location.reload();
}
function funcBtnRed() {
    window.localStorage.setItem("theme", "#003049");
    window.location.reload();
}
...
function renderElem() {
    ...
    data.forEach((elem) => {
        let templateFragment = elTemplate.cloneNode(true);
        templateFragment.querySelector("#images").src = elem.imageURL;
        templateFragment.querySelector("#images").onerror = (e) => {
            e.target.src = "http://via.placeholder.com/266x177";
        };
        templateFragment.querySelector(".card-title").textContent = elem.name;
        templateFragment.querySelector(".card-text").textContent =
            elem.steps.toString().split(" ").slice(0, 15).join(" ") + "...";
        templateFragment.querySelector(".btn-primary").href = elem.originalURL;
        elList.appendChild(templateFragment);
    });
}
